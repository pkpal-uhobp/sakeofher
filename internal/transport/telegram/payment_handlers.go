...
func (b *Bot) handleStarsPayment(ctx context.Context, upd update, rawPriceID string) error {
	...
	description := fmt.Sprintf("SakeOfHer VPN на %s. Лимит: %s. После оплаты доступ будет выдан автоматически.", durationLabel(payment.DurationDays), trafficLabel(payment.TrafficLimitBytes))
...
func (b *Bot) handleSuccessfulPayment(ctx context.Context, upd update) error {
	...
	payment, err := b.services.Payments.MarkPaidForDev(ctx, paymentID, providerPaymentID)
	if err != nil {
...
func (b *Bot) activateWithoutPayment(ctx context.Context, upd update, rawTariffID string) error {
	if !b.settings.AllowFreePurchase {
		return b.replyOrEdit(ctx, upd, "Покупка без оплаты выключена. Для локального теста установи BOT_ALLOW_FREE_PURCHASE=1.", backToMenuKeyboard())
	}
	telegramID := updateTelegramID(upd)
	if telegramID <= 0 {
		return nil
	...
	// CreateManual in the current backend first touches Remnawave with now+tariff days
	// and only then extends the local subscription from the current expiration date.
	// Calling Enable right after it forces Remnawave to receive the final DB expiration
	// date and the default squads. This fixes the case when the bot shows 200+ days,
	// but Remnawave still shows only one month.
