...
func (s *workerService) RetryFailedActivations(ctx context.Context) error {
	if err := s.payments.PollCryptoBotPayments(ctx, 100); err != nil {
		return err
	}
...
func (s *workerService) notifyLowTraffic(ctx context.Context) error {
	...
	for _, item := range items {
		...
		text := fmt.Sprintf(
			"Заканчивается трафик в текущем периоде: осталось примерно %s.\n\nДата обновления трафика: %s.\nЕсли трафик закончится раньше, доступ восстановится после начала нового периода или после продления.",
			label,
			item.Subscription.CurrentPeriodEnd.Format("02.01.2006"),
		)
...
func (s *workerService) notifyTrafficExhaustedDaily(ctx context.Context, now time.Time) error {
	...
	for _, item := range items {
		...
		text := fmt.Sprintf(
			"Трафик в текущем периоде исчерпан.\n\nДоступ временно остановлен. Он восстановится после обновления периода: %s, либо после продления подписки в боте.",
			item.Subscription.CurrentPeriodEnd.Format("02.01.2006"),
		)
...
func trafficLabelAndKey(remainingBytes int64) (string, string) {
	switch {
	case remainingBytes <= 2*domain.BytesInGiB:
		return "2 ГБ", "traffic_2gb"
