...
onMounted(async () => {
  try {
    ...
    error.value = e?.response?.data?.error || e.message || 'Нужно войти в админ-панель'
  } finally {
    loading.value = false
  }
})
...
.error { color: #fca5a5; }
.login-link, .secondary { display: inline-block; margin-top: 18px; border: 0; border-radius: 16px; padding: 14px 18px; font-weight: 800; cursor: pointer; text-decoration: none; }
.login-link { background: #60a5fa; color: #020617; }
